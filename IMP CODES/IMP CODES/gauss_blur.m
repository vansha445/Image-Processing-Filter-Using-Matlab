...
Gx=conv2(double(gray_img),double(k_vedge))
Gy=conv2(double(gray_img),double(k_hedge))
...
subplot(2,3,2)
imshow(abs(G_1),[])
title('Sobel Edge Detection without blur')

subplot(2,3,3)
imshow(abs(G_1),[])
title('Sobel Edge Detection with gausian kernel 3*3 sigma=0.375')

subplot(2,3,4)
imshow(abs(G_2),[])
title('Sobel Edge detection with gausian kernel 6*6 sigma=0.75')

subplot(2,3,5)
imshow(abs(G_3),[])
title('Sobel Edge Detection with gausian kernel 12*12 sigma=1')

subplot(2,3,6)
imshow(abs(G_4),[])
title('Sobel edge detection with gausian kernel 24*24 sigma=3')